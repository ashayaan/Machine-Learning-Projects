...
def benchmark(clf,ind):
    ...
    y_t = preprocessing.label_binarize(y_test,range(len(target_names)))
    fpr,tpr,_ = metrics.roc_curve(y_t.ravel(),y_score.ravel())
    ...
    return ind, score, train_time, test_time,fpr,tpr,roc_auc
...
for penalty in ["l2", "l1"]:
    ...
    results.append(benchmark(SGDClassifier(loss='log',alpha=.0001, n_iter=50,
                                           penalty=penalty,class_weight="balanced"),'SGD'+penalty))
...
results1 = [[x[i] for x in results] for i in range(4)]
...
plt.figure()
for i,c in zip(range(len(results)),['crimson','orange','black','darkblue','plum']):
	name,_,_,_,fpr,tpr,roc_auc = results[i]
	plt.plot(fpr, tpr,label=name + ' micro-average ROC curve (area = {0:0.2f})' 
		''.format(roc_auc),color=c, linestyle=':', linewidth=4)
