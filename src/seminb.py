...
def normalize_p_c(p_c):
    ...
    denom = M + np.sum(p_c)
    p_c += 1.0
    #np.add(p_c, 1.0, out=p_c, casting="unsafe")
    p_c /= denom
...
#	return pd.Series(labels[labels.index[row_num]] == target_names)
...
if __name__=="__main__":
	...
	data_labelled = data.dropna().copy()
	data_labelled = data_labelled.loc[data_labelled['Label'].isin(['crime','diagnostics','video games','demography','trade and business','online information'])]
	data_unlabelled = data[pd.isnull(data['Label'])]
	text_labelled_raw = data_labelled['Name']+data_labelled['Big Description']+data_labelled['Summary']
	text_labelled = text_labelled_raw.apply(stemming)
	text_unlabelled_raw = data_unlabelled['Name']+data_unlabelled['Big Description']+data_unlabelled['Summary']
	text_unlabelled = text_unlabelled_raw.apply(stemming)
	labels = data_labelled['Label'].copy()
	targets = labels.unique()
	vectorizer = TfidfVectorizer(sublinear_tf = True, max_df=0.5,stop_words='english')
	X_train_labelled = vectorizer.fit_transform(text_labelled)
	X_train_unlabelled = vectorizer.transform(text_unlabelled)
	series = pd.Series(np.arange(labels.shape[0]))
	labels_mat = series.apply(lambda r: pd.Series(labels[labels.index[r]] == targets))
	labels_matrix = labels_mat.as_matrix().astype(float)
	X_train = X_train_labelled.transpose()
	X_unlabelled = X_train_unlabelled.transpose()
	print X_train.shape
	print labels_matrix.shape
	print X_unlabelled.shape
	nb_obj = SemiNB()
	model = nb_obj.train_semi(X_train.todense(),labels_matrix,X_unlabelled.todense(),eps=0.00001)
	#print targets[9]
	print nb_obj.predict_all(X_unlabelled.todense())
